...
import { ATELIER_MEDIA_SHOWCASE } from "@/constants/atelierShowcaseData";
...
export function AtelierShowcase() {
    ...
    return (
        <section className="py-24 bg-zinc-950 text-white selection:bg-amber-500/30 border-t border-white/10">
            <div className="container mx-auto px-4 md:px-8 max-w-7xl">
                {/* Header */}
                <div className="mb-12 text-center md:text-left">
                    <h2 className="text-4xl md:text-6xl font-black tracking-tighter mb-4 text-amber-50">
                        CONTENT SHOWCASE
                    </h2>
                    <p className="text-gray-400 text-lg md:text-xl font-light">
                        아틀리에 26의 좌충우돌 라이프, 다양한 미디어로 만나보세요.
                    </p>
                </div>
                    ...
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={`relative px-6 py-4 flex items-center gap-2 text-lg font-bold transition-all
                                ${activeTab === tab.id ? "text-amber-400" : "text-gray-600 hover:text-gray-300"}`}
                        >
...
function VideoTab() {
    const tvcItems = ATELIER_MEDIA_SHOWCASE.filter(item => item.type === 'video_tvc');
    const shortsItems = ATELIER_MEDIA_SHOWCASE.filter(item => item.type === 'video_shorts');
    ...
    return (
        ...
        >
            ...
            <div className="space-y-6">
                <h3 className="text-2xl font-bold tracking-widest text-amber-500 uppercase">Featured & Trailers</h3>
                        ...
                        <div key={item.id} className="group cursor-pointer">
                            <div className="relative aspect-video rounded-xl overflow-hidden bg-zinc-900 border border-white/10 mb-4">
                                ...
                                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div className="w-16 h-16 rounded-full bg-amber-500/90 backdrop-blur border border-white/30 flex items-center justify-center">
                            ...
                            <h4 className="text-xl font-bold mb-1 group-hover:text-amber-400 transition-colors">{item.title}</h4>
            ...
            {/* Shorts Grid */}
            <div className="space-y-6">
                <h3 className="text-2xl font-bold tracking-widest text-amber-500 uppercase">Fun Shorts</h3>
...
function AudioTab() {
    const audioItems = ATELIER_MEDIA_SHOWCASE.filter(item => item.type === 'audio');
        ...
        >
            {audioItems.map((item) => {
                ...
                return (
                    <div
                        key={item.id}
                        className={`group relative bg-zinc-900/50 border ${isPlaying ? 'border-amber-500 bg-zinc-900' : 'border-white/10'} rounded-2xl p-6 md:p-8 flex flex-col md:flex-row gap-8 items-center transition-all hover:bg-zinc-800`}
                    >
                        {/* Thumbnail */}
                        ...
                        <div className="flex-1 w-full flex flex-col justify-center">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <h3 className="text-2xl font-bold text-white mb-1 group-hover:text-amber-400 transition-colors">{item.title}</h3>
                                    <p className="text-gray-400">{item.description}</p>
                                </div>
                                <button
                                    onClick={() => setPlayingId(isPlaying ? null : item.id)}
                                    className="w-12 h-12 rounded-full bg-amber-500 text-white flex items-center justify-center hover:scale-110 transition-transform"
                                >
                            ...
                            <div className="h-12 flex items-end gap-1 opacity-50">
                                {[...Array(40)].map((_, i) => (
                                    <motion.div
                                        key={i}
                                        className={`w-1 bg-current rounded-t-sm ${isPlaying ? 'text-amber-500' : 'text-gray-600'}`}
...
function SocialTab() {
    const socialItems = ATELIER_MEDIA_SHOWCASE.filter(item => item.type === 'social_ugc');
...
function VisualTab() {
    const visualItems = ATELIER_MEDIA_SHOWCASE.filter(item => item.type === 'image_poster');
